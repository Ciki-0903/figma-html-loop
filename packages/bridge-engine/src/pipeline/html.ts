...
function renderWrapperBox(cfg: RenderBoxConfig): string {
  ...
  if (opts?.mode === 'content') {
    innerAttrs['data-figma-id'] = id;
    if (opts?.hasStroke) innerAttrs['data-layer-id'] = id;
  }
...
async function renderTextNode(ctx: RenderContext): Promise<string> {
  ...
  // Single-line text (badges / pills) is centred in the browser by the font's CJK
  // line metrics and drifts visibly low inside a bordered container. Detect a
  // one-line box (no newline, height within ~2 lines) so we can centre it by the
  // box's own flex alignment and drop the per-run line-height further below.
  let singleLineText = false;
  let singleLineBoxH = 0;
  let singleLineFontSize = 0;
  if (ctx.mode === 'content' && ctx.irNode.text) {
    const t: any = ctx.irNode.text;
    const chars = typeof t.characters === 'string' ? t.characters : '';
    const seg0 = Array.isArray(t.segments) && t.segments[0] ? t.segments[0] : null;
    const fs = seg0 && typeof seg0.fontSize === 'number' ? seg0.fontSize : 16;
    const h = ctx.irNode.layout && typeof (ctx.irNode.layout as any).height === 'number' ? (ctx.irNode.layout as any).height : 0;
    if (chars && chars.indexOf('\n') === -1 && h > 0 && h <= fs * 2.2) {
      singleLineText = true;
      singleLineBoxH = h;
      singleLineFontSize = fs;
    }
  }
  ...
  if (ctx.mode === 'debug') {
    ...
    textHtml = await renderTextSegmentsWithClasses(ctx.irNode.text, ctx.usedClasses, { suppressLineHeight: singleLineText });
  } else {
  ...
  // Single-line text (badges / pills) hugs its box to the browser's CJK line box,
  // which runs taller than Figma's box — so the text overflows a tight container
  // downward and looks low or clipped. Pin the run's line-height to Figma's own box
  // height (line-height suppressed on the run above so it inherits this): the hug box
  // then equals the Figma height, sits where Figma placed it, and the glyph centres
  // within that line box — matching Figma instead of drifting below the container.
  if (singleLineText && singleLineBoxH > 0) {
    // font-size on the box makes its strut match the text so line-height governs the
    // box height exactly (a default 16px strut would otherwise inflate a small badge).
    if (singleLineFontSize > 0) boxCss += `font-size:${singleLineFontSize}px;`;
    boxCss += `line-height:${singleLineBoxH}px;`;
  }
