...
export function segmentToInlineCss(
  seg: FigmaText['segments'] extends (infer S)[] ? S & Record<string, any> : any,
  opts?: { suppressLineHeight?: boolean }
): string {
  const parts: string[] = [];
  ...
  // Single-line badge/pill text centers by the box's flex alignment instead, so the
  // per-run line-height is suppressed there — otherwise the browser distributes the
  // line-height leading by CJK font metrics and the glyph sits visibly low.
  if (seg.lineHeight && !(opts && opts.suppressLineHeight)) {
...
export async function renderTextSegmentsWithClasses(
  ...
  for (const seg of segments) {
    ...
    const end = seg.end || chars.length;
    const slice = chars.slice(start, end);
    const css = segmentToInlineCss(seg, opts);
    const html = escapeHtml(slice).replace(/\n/g, '<br>');
